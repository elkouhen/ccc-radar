...
from codeatlas.flow import FlowResult
...
from codeatlas.search import SearchHit, Summary, get_context
from codeatlas.workspace import DiscoveredService, FederationResult


class FindingHit(TypedDict):
    """Shape returned by `codeatlas search --json` and the `search_findings` MCP tool."""
...
class FindingsSummary(TypedDict):
    """Shape returned by `codeatlas summary --json` and the `findings_summary` MCP tool."""
...
class GraphResult(TypedDict):
    """Shape returned by `codeatlas export microservices --json` and the MCP `graph` tool.
...
def render_graph_likec4(
    ...
    lines = [
        "// Generated by codeatlas export microservices --c4. Do not edit generated identifiers.",
        "specification {",
...
<body>
  ...
  <script type="module">
    ...
    const pathHistoryStorageKey = (() => {
      ...
      return `codeatlas:analyzed-paths:${hash >>> 0}`;
    })();
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>CodeAtlas module dependencies</title>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/graphology/0.25.4/graphology.umd.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/sigma.js/2.4.0/sigma.min.js"></script>
  <style>
    :root { color: #172033; background: #f5f7fb; font-family: Inter, ui-sans-serif, system-ui, sans-serif; }
...
def _openapi_contract_evidence_path(endpoint: MessageEndpoint) -> str:
    ...
    for line in endpoint.snippet.splitlines():
        if line.startswith("codeatlas-openapi-contract:"):
            return line.removeprefix("codeatlas-openapi-contract:")
...
class WorkspaceResult(TypedDict):
    """Shape returned by `codeatlas microservices [--root ROOT] --json` and the
