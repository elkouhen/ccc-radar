...
(() => {
  const rawData = JSON.parse(document.getElementById('runtime-data').textContent);
  const data = Object.assign({services: [], transactions: [], dependencies: [], timeline_spans: [], distributed_traces: [], coverage: {}}, rawData);
  data.coverage = Object.assign({services: {}, transactions: {}, dependencies: {}, timeline: {}, distributed_traces: {}}, data.coverage || {});
...
<header><h1>APM runtime overview</h1><p class="subtle">Bounded aggregates plus sampled cross-service traces. Selecting an event in a trace exposes its transaction or span ID; request data, headers, bodies and raw error messages are excluded.</p><label>Window <select id="runtime-window-select" aria-label="Select runtime window"><option value="15m">Last 15 minutes</option><option value="1h">Last hour</option><option value="6h">Last 6 hours</option><option value="24h">Last 24 hours</option><option value="7d">Last 7 days</option></select></label><div id="context" class="context"></div><div id="report-status" class="report-status"></div></header>
...
const rawData=JSON.parse(document.getElementById('runtime-data').textContent); const data=Object.assign({window:{from:'—',to:'—'},services:[],transactions:[],dependencies:[],timeline_spans:[],distributed_traces:[],coverage:{}},rawData);data.coverage=Object.assign({services:{},transactions:{},dependencies:{},timeline:{},distributed_traces:{}},data.coverage||{}); const $=id=>document.getElementById(id); const n=v=>typeof v==='number'?v:null; const ms=v=>n(v)===null?'—':`${v.toLocaleString(undefined,{maximumFractionDigits:3})} ms`; const pct=v=>n(v)===null?'—':`${(v*100).toFixed(1)}%`; const count=v=>n(v)===null?'—':v.toLocaleString(); const esc=v=>String(v??'—').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
const runtimeWindow=$('runtime-window-select');runtimeWindow.value=data.selected_window||'1h';runtimeWindow.addEventListener('change',()=>{const url=new URL(location.href);url.searchParams.set('since',runtimeWindow.value);location.assign(url);});
