...
(() => {
  ...
  function selectView(view) {
    ...
    ['overview', 'details', 'timeline'].forEach(name => byId(`${name}-tab`).classList.toggle('is-active', name === view));
  }
  ['overview', 'details', 'timeline'].forEach(name => byId(`${name}-tab`).addEventListener('click', () => selectView(name)));
...
#overview-layout { display:grid; grid-template-columns:minmax(240px,.62fr) minmax(0,1.8fr); gap:12px; align-items:start; } .overview-sidebar { display:grid; gap:12px; } .overview-sidebar .panel { margin-top:0; } .overview-sidebar .triage-items { grid-template-columns:1fr; } .map-panel { display:grid; grid-template-columns:minmax(0,1fr) minmax(250px,.42fr); gap:12px; align-items:start; margin-top:0; overflow:visible; } .map-panel .panel-head { grid-column:1/-1; } .map-panel .service-map { grid-column:1; } .map-panel .service-map-details { grid-column:2; grid-row:2; grid-template-columns:1fr; margin-top:0; max-height:410px; overflow:auto; } @media (max-width:1050px) { #overview-layout { grid-template-columns:1fr; } .overview-sidebar { grid-template-columns:minmax(0,1fr) minmax(260px,.85fr); } .filter-bar { margin:0; } } @media (max-width:800px) { .overview-sidebar,.map-panel { grid-template-columns:1fr; } .map-panel .service-map,.map-panel .service-map-details { grid-column:1; grid-row:auto; } .map-panel .service-map-details { max-height:none; } }
